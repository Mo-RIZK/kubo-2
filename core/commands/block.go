...
var blockGetNoStoreCmd = &cmds.Command{
	Helptext: cmds.HelpText{
		Tagline: "Get UnixFS leaf data without storing the block locally.",
		ShortDescription: `
...
`,
	...
	Run: func(
		...
		default:
			return fmt.Errorf(
				"unsupported CID codec %d for CID %s",
				c.Type(),
				c,
			)
		}

		return res.Emit(files.NewBytesFile(nodeData))
	},
}
